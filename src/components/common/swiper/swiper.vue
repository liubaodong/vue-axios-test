...
export default {
    ...
    mounted() {
        ...
        setTimeout(() => {
            ...
            this.dom["-webkit-transform"] = `translate3d(${this._width *
                -1}px, 0px, 0px)`;
            this.dom["-ms-transform"] = `translate3d(${this._width *
                -1}px, 0px, 0px)`;
            if (this.autoPlay) {
                this.setTime();
            }
    ...
    methods: {
        ...
        getTransform() {
            var x =
                this.dom.transform ||
                this.dom["-webkit-transform"] ||
                this.dom["-ms-transform"];
            x = x.substring(12);
            x = x.match(/(\S*)px/)[1];
            return Number(x);
        },
        ...
        starDom() {
            ...
            if (this.slidesLength > 1) {
                ...
                document
                    .querySelector("." + this.className)
                    .appendChild(cloneDom1);
                this._width = document.querySelector(
                    "." + this.className
                ).offsetWidth;
